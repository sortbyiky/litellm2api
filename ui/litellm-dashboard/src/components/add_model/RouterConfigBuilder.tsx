  ...
  return (
    <div className="w-full max-w-none">
      <Flex justify="space-between" align="center" gap="middle" style={{ width: "100%", marginBottom: 24 }}>
        <Space align="center">
          <Typography.Title level={4} style={{ margin: 0 }}>{t("addModel.routesConfiguration")}</Typography.Title>
          <Tooltip title={t("addModel.routesConfigurationTooltip")}>
      ...
      {routes.length === 0 ? (
        <Card>
          <Empty description={t("addModel.noRoutesConfigured")} />
        </Card>
      ) : (
        <Collapse
          ...
          items={routes.map((route, index) => ({
            ...
            label: (
              <Text style={{ fontSize: 16 }}>
                Route {index + 1}: {route.model || t("addModel.unnamed")}
              </Text>
            ),
            extra: (
              <Button
            ...
            children: (
              <Card key={route.id}>
                ...
                <div className="mb-4 w-full">
                  <Text className="text-sm font-medium mb-2 block">{t("addModel.model")}</Text>
                  <AntdSelect
                    value={route.model}
                    onChange={(value) => updateRoute(route.id, "model", value)}
                    placeholder={t("addModel.selectModel")}
                    showSearch
                    style={{ width: "100%" }}
                ...
                {/* Description */}
                <div className="mb-4 w-full">
                  <Text className="text-sm font-medium mb-2 block">{t("addModel.description")}</Text>
                  <TextArea
                    value={route.description}
                    onChange={(e) => updateRoute(route.id, "description", e.target.value)}
                    placeholder={t("addModel.describeRouteUsage")}
                    rows={2}
                    style={{ width: "100%" }}
                  />
                </div>
                ...
                <div className="mb-4 w-full">
                  <div className="flex items-center gap-2 mb-2">
                    <Text className="text-sm font-medium">{t("addModel.scoreThreshold")}</Text>
                    <Tooltip title={t("addModel.scoreThresholdTooltip")}>
                      <InfoCircleOutlined className="text-gray-400" />
                    </Tooltip>
                  </div>
                  <InputNumber
                ...
                <div className="w-full">
                  <div className="flex items-center gap-2 mb-2">
                    <Text className="text-sm font-medium">{t("addModel.exampleUtterances")}</Text>
                    <Tooltip title={t("addModel.exampleUtterancesTooltip")}>
                      <InfoCircleOutlined className="text-gray-400" />
                    </Tooltip>
                  </div>
                  <Text className="text-xs text-gray-500 mb-2">
                    {t("addModel.utterancesHint")}
                  </Text>
                  <AntdSelect
                    mode="tags"
                    value={route.utterances}
                    onChange={(utterances) => updateRoute(route.id, "utterances", utterances)}
                    placeholder={t("addModel.utterancePlaceholder")}
                    style={{ width: "100%" }}
                    tokenSeparators={["\n"]}
      ...
      <div className="flex justify-between items-center mb-4 w-full">
        <Text className="text-lg font-semibold">{t("addModel.jsonPreview")}</Text>
        <Button type="link" onClick={() => setShowJsonPreview(!showJsonPreview)} className="text-blue-600 p-0">
          {showJsonPreview ? t("addModel.hide") : t("addModel.show")}
        </Button>
      </div>
