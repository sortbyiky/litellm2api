  ...
  return (
    <>
      {isLoading && allFields.length === 0 && (
        <Row>
          <Col span={24}>
            <Text className="mb-2">{t("addModel.loadingProviderFields")}</Text>
          </Col>
        </Row>
      )}
        ...
        <Row>
          <Col span={24}>
            <Text className="mb-2 text-red-500">
              {loadError instanceof Error ? loadError.message : t("addModel.failedToLoadProviderFields")}
            </Text>
          </Col>
        </Row>
      )}
      {allFields.map((field) => (
        <React.Fragment key={field.key}>
          <Form.Item
            label={field.label}
            name={field.key}
            rules={field.required ? [{ required: true, message: t("addModel.required") }] : undefined}
          ...
          >
            {field.type === "select" ? (
              ...
              >
                <Button2 icon={<UploadOutlined />}>{t("addModel.clickToUpload")}</Button2>
              </Upload>
            ) : field.type === "textarea" ? (
              <Input.TextArea
          ...
          {field.key === "vertex_credentials" && (
            <Row>
              <Col>
                <Text className="mb-3 mt-1">{t("addModel.gcpServiceAccountHint")}</Text>
              </Col>
            </Row>
          )}
