...
export const healthCheckColumns = (
...
): ColumnDef<HealthCheckData>[] => [
  {
    header: () => (
      <div className="flex items-center gap-2">
        ...
        />
        <span>{t("modelDashboard.modelId")}</span>
      </div>
    ),
  ...
  {
    header: t("modelDashboard.modelName"),
  ...
  {
    header: t("modelDashboard.teamAlias"),
  ...
  {
    header: t("modelDashboard.healthStatus"),
      ...
      if (healthStatus.loading) {
        return (
          <div className="flex items-center space-x-2">
            ...
            <Text className="text-gray-600 text-sm">{t("modelDashboard.checking")}</Text>
          </div>
        );
      }
      ...
      return (
        <div className="flex items-center space-x-2">
          ...
          {hasSuccessResponse && showSuccessModal && (
            <Tooltip title={t("modelDashboard.viewResponseDetails")} placement="top">
              <button
  ...
  {
    header: t("modelDashboard.errorDetailsHeader"),
    ...
    cell: ({ row }) => {
      ...
      if (!healthStatus?.error) {
        return <Text className="text-gray-400 text-sm">{t("modelDashboard.noErrors")}</Text>;
      }
      ...
      return (
        <div className="flex items-center space-x-2">
          ...
          {showErrorModal && fullError !== cleanedError && (
            <Tooltip title={t("modelDashboard.viewFullErrorDetails")} placement="top">
              <button
  ...
  {
    header: t("modelDashboard.lastCheck"),
    ...
    cell: ({ row }) => {
      ...
      return (
        <Text className="text-gray-600 text-sm">
          {model.health_loading ? t("modelDashboard.checkInProgress") : model.last_check}
        </Text>
      );
    },
  },
  {
    header: t("modelDashboard.lastSuccess"),
  ...
  {
    header: t("modelDashboard.actions"),
    id: "actions",
    cell: ({ row }) => {
      const model = row.original;
      const modelId = model.model_info?.id ?? "";
      ...
      const tooltipText = model.health_loading
        ? t("modelDashboard.checking")
        : hasExistingStatus
          ? t("modelDashboard.reRunHealthCheck")
          : t("modelDashboard.runHealthCheck");
