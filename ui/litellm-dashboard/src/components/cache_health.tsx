    ...
    <div className="bg-white rounded-lg shadow">
      <TabGroup>
        <TabList className="border-b border-gray-200 px-4">
          <Tab className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">{t("cache.summary")}</Tab>
          <Tab className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">{t("cache.rawResponse")}</Tab>
        </TabList>
          ...
          <TabPanel className="p-4">
            <div>
              <div className="flex items-center mb-6">
                ...
                >
                  {t("cache.cacheStatus")}: {response?.status || "unhealthy"}
                </Text>
              </div>
              ...
              <table className="w-full border-collapse">
                <tbody>
                    ...
                    <>
                      <tr>
                        <td colSpan={2} className="pt-4 pb-2 font-semibold text-red-600">
                          {t("cache.errorDetails")}
                        </td>
                      </tr>
                      <TableClickableErrorField label={t("cache.errorMessage")} value={errorDetails.message} />
                      <TableClickableErrorField label={t("cache.traceback")} value={errorDetails.traceback} />
                    </>
                  )}
                  ...
                  <TableClickableErrorField label={t("cache.cacheConfiguration")} value={String(parsedLitellmParams?.type)} />
                  <TableClickableErrorField label={t("cache.pingResponse")} value={String(response.ping_response)} />
                  <TableClickableErrorField label={t("cache.setCacheResponse")} value={response.set_cache_response || "N/A"} />
                  ...
                  {parsedLitellmParams?.type === "redis" && (
                    <>
                      <tr>
                        <td colSpan={2} className="pt-4 pb-2 font-semibold">
                          {t("cache.redisDetails")}
                        </td>
                      </tr>
                      <TableClickableErrorField label={t("cache.redisHost")} value={redisDetails.redis_host || "N/A"} />
                      <TableClickableErrorField label={t("cache.redisPort")} value={redisDetails.redis_port || "N/A"} />
                      <TableClickableErrorField label={t("cache.redisVersion")} value={redisDetails.redis_version || "N/A"} />
                      <TableClickableErrorField label={t("cache.startupNodes")} value={redisDetails.startup_nodes || "N/A"} />
                      <TableClickableErrorField label={t("cache.namespace")} value={redisDetails.namespace || "N/A"} />
                    </>
                  )}
                </tbody>
              </table>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        >
          {isLoading ? t("cache.runningHealthCheckBtn") : t("cache.runHealthCheck")}
        </Button>
