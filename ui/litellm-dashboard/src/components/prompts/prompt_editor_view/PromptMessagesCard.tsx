...
const PromptMessagesCard: React.FC<PromptMessagesCardProps> = ({
  ...
  return (
    <Card className="p-3">
      <div className="mb-2">
        <Text className="text-sm font-medium">{t("prompts.promptMessages")}</Text>
        <Text className="text-gray-500 text-xs mt-1">
          {t("prompts.promptMessagesDesc")}
        </Text>
      </div>
          ...
          >
            <div className="bg-gray-50 px-2 py-1.5 border-b border-gray-300 flex items-center justify-between">
              <Select
                ...
                bordered={false}
              >
                <Option value="user">{t("prompts.roleUser")}</Option>
                <Option value="assistant">{t("prompts.roleAssistant")}</Option>
                <Option value="system">{t("prompts.roleSystem")}</Option>
              </Select>
            ...
            <div className="p-2">
              <VariableTextArea
                ...
                rows={3}
                placeholder={t("prompts.enterPromptContent")}
              />
            </div>
          </div>
        ))}
