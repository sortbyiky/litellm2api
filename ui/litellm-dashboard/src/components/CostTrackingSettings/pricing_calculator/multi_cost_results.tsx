...
const SingleModelBreakdown: React.FC<{
  ...
  const periodLabel = timePeriod === "day" ? t("costTracking.daily") : t("costTracking.monthly");
    ...
    <div className="space-y-3 bg-gray-50 p-4 rounded-lg">
      {loading && (
        <div className="flex items-center gap-2 text-gray-500 text-sm">
          ...
          <span>{t("costTracking.updating")}</span>
        </div>
      )}

      <div className="grid grid-cols-4 gap-4">
        <div>
          <Text className="text-xs text-gray-500 block">{t("costTracking.totalPerRequest")}</Text>
          <Text className="text-base font-semibold text-blue-600">{formatCost(result.cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("costTracking.inputCost")}</Text>
          <Text className="text-sm">{formatCost(result.input_cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("costTracking.outputCost")}</Text>
          <Text className="text-sm">{formatCost(result.output_cost_per_request)}</Text>
        </div>
        <div>
          <Text className="text-xs text-gray-500 block">{t("costTracking.marginFee")}</Text>
      ...
      {periodCost !== null && (
        <div className="grid grid-cols-4 gap-4 pt-2 border-t border-gray-200">
          <div>
            <Text className="text-xs text-gray-500 block">{periodLabel} {t("costTracking.total")} ({formatRequests(periodRequests)} req)</Text>
            <Text className={`text-base font-semibold ${timePeriod === "day" ? "text-green-600" : "text-purple-600"}`}>
              {formatCost(periodCost)}
            </Text>
          </div>
          <div>
            <Text className="text-xs text-gray-500 block">{periodLabel} {t("costTracking.input")}</Text>
            <Text className="text-sm">{formatCost(periodInputCost)}</Text>
          </div>
          <div>
            <Text className="text-xs text-gray-500 block">{periodLabel} {t("costTracking.output")}</Text>
            <Text className="text-sm">{formatCost(periodOutputCost)}</Text>
          </div>
          <div>
            <Text className="text-xs text-gray-500 block">{periodLabel} {t("costTracking.marginFee")}</Text>
...
const MultiCostResults: React.FC<MultiCostResultsProps> = ({ multiResult, timePeriod }) => {
    ...
    return (
      <div className="py-6 text-center border border-dashed border-gray-300 rounded-lg bg-gray-50">
        <Text className="text-gray-500">
          {t("costTracking.selectModelsToSee")}
        </Text>
      </div>
    );
  }
  ...
  if (!hasAnyResult && isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center">
        ...
        <Text className="text-gray-500 block mt-2">{t("costTracking.calculatingCosts")}</Text>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <Text className="text-base font-semibold text-gray-900">{t("costTracking.costEstimates")}</Text>
        ...
        {errorEntries.map((e) => (
          <div key={e.entry.id} className="text-sm text-red-600 bg-red-50 p-3 rounded-lg border border-red-200">
            <span className="font-medium">{e.entry.model || t("costTracking.unknownModel")}: </span>
            {e.error}
          </div>
        ))}
      </div>
  ...
  const summaryColumns = [
    ...
    {
      title: t("costTracking.perRequest"),
    ...
    {
      title: t("costTracking.marginFee"),
    ...
    <div className="space-y-4">
      ...
      <Card size="small" className="bg-gradient-to-r from-slate-50 to-blue-50 border-slate-200">
        <Row gutter={[16, 8]}>
          <Col xs={24} sm={12}>
            <Statistic
              title={<span className="text-xs">{t("costTracking.totalPerRequest")}</span>}
          ...
          <Col xs={24} sm={12}>
            <Statistic
              title={<span className="text-xs">{t("costTracking.total")} {periodLabel}</span>}
        ...
        {hasMargin && (
          <Row gutter={[16, 8]} className="mt-3 pt-3 border-t border-slate-200">
            <Col xs={24} sm={12}>
              <div className="text-xs text-gray-500">{t("costTracking.marginFeePerRequest")}</div>
              <div className="text-sm font-mono text-amber-600">{formatCost(multiResult.totals.margin_per_request)}</div>
            </Col>
            <Col xs={24} sm={12}>
              <div className="text-xs text-gray-500">{periodLabel} {t("costTracking.marginFee")}</div>
